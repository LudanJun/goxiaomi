...
func (con RoleController) Edit(c *gin.Context) {
	id,err :=  models.StringToInt(c.Query("id"))//获取的id 是string类型
...
func (con RoleController) DoEdit(c *gin.Context) {
		//获取表单数据 
		id,err1 := models.StringToInt(c.PostForm("id")) //获取id
		if err1 != nil {
			con.Error(c,"传入参数错误","/admin/role") 
			return
		}
		...
		if err2 != nil {
			con.Error(c,"修改数据失败请重试","/admin/role/edit?id="+models.IntToString(id)) 
		}else {
			con.Success(c,"修改数据成功","/admin/role/edit?id="+models.IntToString(id))
		}
		

}
func (con RoleController) Delete(c *gin.Context) {
	id,err1 := models.StringToInt(c.Query("id")) //获取id
	if err1 != nil {
		con.Error(c,"传入参数错误","/admin/role") 
